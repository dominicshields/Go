...
N.B. IF THE FILES ARE NOT SORTED ON THE FIELDS/BYTE POSITION OF THE INPUT MATCH POSITIONS THIS WILL FAIL TO PRODUCE CORRECT OUTPUT FOR OBVIOUS REASONS.
     ...
     KNOWN ISSUE - IF THE LAST RECORD IN THE FILE DOES NOT HAVE A NEWLINE IT WILL FAIL TO BE READ, THIS SEEMS TO BE A LIMITATION OF GO, THERE WAS NOT
     ANY READ FILE METHOD I COULD FIND THAT HAD THE SIMPLICITY AND VERSATILITY OF FGETS IN C.
     ...
     HAS AN EXECUTABLE WHICH IS 2065KB - WHEN PEOPLE TALK ABOUT C BEING OUTDATED I SAY THEY DON'T KNOW THE FIRST THING ABOUT PROGRAMMING.
Dominic Shields 06/03/2018 23:16:51 */
...
func main() {
	...
	for file1EOF != 9 && file2EOF != 9 { // A BIT OF A HACK AS GO DOES NOT HAVE THE RICH SET OF FILE OPERATIONS THAT C DOES
		...
		if infile1cmp > infile2cmp {
			fmt.Fprintf(in2not1, "%v", infile2buf)
			in2not1cnt++
			infile2buf, _ = readfile2(infile2ptr)

		} else if infile1cmp < infile2cmp {
			fmt.Fprintf(in1not2, "%v", infile1buf)
			in1not2cnt++
			infile1buf, _ = readfile1(infile1ptr)

		} else if infile1cmp == infile2cmp {
			fmt.Fprintf(matches1, "%v", infile1buf)
			fmt.Fprintf(matches2, "%v", infile2buf)
			match1cnt++
			match2cnt++
			infile1buf, _ = readfile1(infile1ptr)
			infile2buf, _ = readfile2(infile2ptr)
...
func readfile1(infile1ptr *bufio.Reader) (string, error) {
	var infile1buf string
	var infile1err error
	infile1buf, infile1err = infile1ptr.ReadString(10) // 0x0A separator = newline
	file1cnt++
...
func readfile2(infile2ptr *bufio.Reader) (string, error) {
	var infile2buf string
	var infile2err error
	infile2buf, infile2err = infile2ptr.ReadString(10) // 0x0A separator = newline
	file2cnt++
